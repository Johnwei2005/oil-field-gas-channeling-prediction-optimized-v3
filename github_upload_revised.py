...
# -*- coding: utf-8 -*-
"""
CCUS CO2气窜预测系统 - GitHub仓库创建和代码上传工具 (修订版)
...
# 设置日志
log_filename = f"github_upload_revised_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def delete_github_repository(token, username, repo_name):
    """
    删除GitHub仓库
    ...
    Returns:
        bool: 是否成功删除
    """
    logger.info(f"尝试删除GitHub仓库: {username}/{repo_name}")
    ...
    try:
        ...
        if response.status_code == 204:
            logger.info(f"GitHub仓库删除成功: {username}/{repo_name}")
            return True
        else:
            logger.warning(f"GitHub仓库删除失败: {response.status_code} - {response.text}")
            return False
    
    except requests.exceptions.RequestException as e:
        logger.error(f"删除GitHub仓库时发生错误: {e}")
        return False
...
def create_github_repository(token, repo_name, description=""):
    ...
    try:
        ...
        return True, repo_info['html_url'], repo_info['owner']['login']
...
def setup_git_repository(repo_url, local_dir, token, username):
    """
    设置本地Git仓库并关联到GitHub
    ...
    try:
        ...
        # 设置Git用户信息
        subprocess.run(["git", "config", "user.email", "johnwei2005@example.com"], cwd=local_dir, check=True)
        subprocess.run(["git", "config", "user.name", "Johnwei2005"], cwd=local_dir, check=True)
...
def create_and_upload_repository(token, repo_name, source_dir, description=""):
    ...
    try:
        ...
        logger.info(f"获取到GitHub用户名: {username}")
    except Exception as e:
        logger.error(f"获取GitHub用户信息失败: {e}")
        return False, None
    
    # 尝试删除已存在的仓库
    delete_github_repository(token, username, repo_name)
    ...
    # 创建GitHub仓库
    success, repo_url, owner = create_github_repository(token, repo_name, description)
    if not success:
        return False, None
    
    # 设置本地Git仓库
    if not setup_git_repository(repo_url, temp_dir, token, username):
